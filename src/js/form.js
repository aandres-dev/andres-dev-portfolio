...
function initContactForm(root, options = {}) {
  ...
  const submitLabel = form?.querySelector('[type="submit"] span[data-es]');
  ...
  function restoreSubmit() {
    if (!submitLabel?.dataset?.restore) return;
    submitLabel.textContent = submitLabel.dataset.restore;
    delete submitLabel.dataset.restore;
  }
  ...
  function setStatus(key) {
    ...
    if (key === 'success' && submitLabel && submitLabel.dataset) {
      submitLabel.dataset.restore = submitLabel.textContent;
      submitLabel.textContent = SENT[lang()];
      hideTimer = setTimeout(clearStatus, 3200);
      hideTimer.unref?.();
    }
  }
